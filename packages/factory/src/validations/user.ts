...
const isAscii = (value: string) =>
  [...value].every((char) => char.charCodeAt(0) <= 0x7f);

const isMultibyte = (value: string) =>
  [...value].every((char) => char.charCodeAt(0) > 0x7f);
    ...
    if (nationality === Nationality.US) {
      if (!isAscii(value)) {
        return this.createError({
          message: "For US, name must contain only 1-byte (ASCII) characters.",
        });
      }
      return true;
    }
    if (nationality === Nationality.JAPAN) {
      if (!isMultibyte(value)) {
        return this.createError({
          message: "For Japan, name must contain only multi-byte characters.",
        });
      }
      return true;
    }
